...
function closeGame() {
    ...
    setTimeout(() => showNotification("Great game! Check out today's competitions!", 'info'), 500);
}

// =====================
// Ads
// =====================
function showInterstitialAd() { showNotification("📱 [Interstitial Ad Shown]", 'info'); }
function showRewardedAd() { showNotification("🎬 [Rewarded Video Ad] +0.1 TON bonus!", 'success'); }
...
function showNotification(message, type='info') {
    ...
    switch(type) {
        ...
        case 'info': default: notification.style.background = 'linear-gradient(45deg, #667eea, #764ba2)'; break;
    }
...
// User profile
// =====================
function showUserProfile() { showNotification('Please login with Telegram to view profile', 'info'); }
...
function saveWallet() {
    ...
    if (!wallet) { statusDiv.textContent = "Please enter a wallet address!"; statusDiv.className="wallet-status-error"; return; }
    if (!wallet.startsWith("EQ") && !wallet.startsWith("UQ")) { statusDiv.textContent="Invalid TON wallet format!"; statusDiv.className="wallet-status-error"; return; }
    if (wallet.length < 40) { statusDiv.textContent="TON wallet address is too short!"; statusDiv.className="wallet-status-error"; return; }

    localStorage.setItem("userWallet", wallet);
    statusDiv.textContent="✅ Wallet saved successfully!"; statusDiv.className="wallet-status-success";
...
async function onTelegramAuth(user) {
    try {
        ...
        showNotification("Authentication failed. Try again.", "error");
    }
}
...
async function checkSession() {
    try {
        const res = await fetch(`${API_URL}/auth/check`, { method:"GET", credentials:"include" });
        if(res.ok) {
            const data = await res.json();
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    setTimeout(()=>console.log('Ad impressions: 12, RPM: $3.20, Revenue: $0.038'),5000);
});
...
function initMobileMenu(){
    ...
    mobileToggle.addEventListener('click',()=>{ mobileToggle.classList.toggle('active'); navLinks.classList.toggle('mobile-open'); });

    const navLinksItems = navLinks.querySelectorAll('a');
    navLinksItems.forEach(link=>link.addEventListener('click',()=>{ mobileToggle.classList.remove('active'); navLinks.classList.remove('mobile-open'); }));

    document.addEventListener('click', e=>{ if(!nav.contains(e.target)){ mobileToggle.classList.remove('active'); navLinks.classList.remove('mobile-open'); } });
}
...
function optimizeForTouch(){
    ...
    inputs.forEach(input=>{
        input.addEventListener('focus',()=>{ if(window.innerWidth<768){ document.querySelector('meta[name=viewport]').setAttribute('content','width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0'); }});
        input.addEventListener('blur',()=>{ if(window.innerWidth<768){ document.querySelector('meta[name=viewport]').setAttribute('content','width=device-width, initial-scale=1.0'); }});
    });
}

// =====================
// Window & orientation handlers
// =====================
...
window.addEventListener('scroll',()=>{
    ...
    if(header) header.style.background = window.scrollY>100 ? 'rgba(0,0,0,0.95)' : 'rgba(0,0,0,0.9)';
});

// =====================
// Game score submission (unchanged)
// =====================
window.addEventListener('message', function(event) {
    if (event.data.type === 'gameComplete') {
        ...
        if(validateScore(game, score, data)){
            ...
            }).then(r=>{ if(r.ok) showNotification(`Score ${score} saved for ${game}!`, 'success'); });
        }
    }
});
